...
class Base {
     ...
     */
    setOptions(...options) {
        Vue.set(this, '_options', defaultsDeep({}, ...options, // Given options
    ...
    getRoute(key, fallback) {
        let route = get(this.routes(), key, fallback ? get(this.routes(), fallback) : undefined);
        if (!route) {
