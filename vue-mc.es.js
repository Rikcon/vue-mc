...
function getFuncName(func) {
  var result = (func.name + ''),
      ...
      length = hasOwnProperty$d.call(_realNames, result) ? array.length : 0;
 ...
 */
const autobind = function (instance) {
...
var assign = _createAssigner(function(object, source) {
  ...
  for (var key in source) {
    if (hasOwnProperty$i.call(source, key)) {
...
(function (RequestOperation) {
    RequestOperation[RequestOperation["REQUEST_CONTINUE"] = 0] = "REQUEST_CONTINUE";
    RequestOperation[RequestOperation["REQUEST_REDUNDANT"] = 1] = "REQUEST_REDUNDANT";
    RequestOperation[RequestOperation["REQUEST_SKIP"] = 2] = "REQUEST_SKIP";
...
class Base {
     ...
     */
    emit(event, context = {}) {
        let listeners = get_1(this._listeners, event);
        if (!listeners) {
            return;
        }
        // Create the context for the event.
        context = defaults_1({}, context, this.getDefaultEventContext());
     ...
     */
    on(event, listener) {
        let events = map_1(split_1(event, ','), trim_1);
        each(events, (event) => {
     ...
     */
    setOptions(...options) {
        Vue.set(this, '_options', defaultsDeep_1({}, ...options, // Given options
    ...
    getDefaultRouteResolver() {
        return (route, parameters = {}) => {
            ...
            return reduce_1(replacements, (result, value, parameter) => {
    ...
    getRoute(key, fallback) {
        let route = get_1(this.routes(), key, fallback ? get_1(this.routes(), fallback) : undefined);
        if (!route) {
    ...
    fetch(options = {}) {
        let config = () => {
            return {
                url: defaultTo_1(options.url, this.getFetchURL()),
                method: defaultTo_1(options.method, this.getFetchMethod()),
                params: defaults_1(options.params, this.getFetchQuery()),
                headers: defaults_1(options.headers, this.getFetchHeaders()),
            };
        };
    ...
    save(options = {}) {
        let config = () => {
            return {
                url: defaultTo_1(options.url, this.getSaveURL()),
                method: defaultTo_1(options.method, this.getSaveMethod()),
                data: defaultTo_1(options.data, this.getSaveData()),
                params: defaultTo_1(options.params, this.getSaveQuery()),
                headers: defaultTo_1(options.headers, this.getSaveHeaders()),
            };
        };
     ...
     */
    upload(options = {}) {
        let data = defaultTo_1(options.data, this.getSaveData());
        let config = () => assign_1(options, {
    ...
    delete(options = {}) {
        let config = () => {
            return {
                url: defaultTo_1(options.url, this.getDeleteURL()),
                method: defaultTo_1(options.method, this.getDeleteMethod()),
                data: defaultTo_1(options.data, this.getDeleteBody()),
                params: defaultTo_1(options.params, this.getDeleteQuery()),
                headers: defaultTo_1(options.headers, this.getDeleteHeaders()),
            };
        };
        return this.request(config, this.onDelete, this.onDeleteSuccess, this.onDeleteFailure);
    }
}
Base.REQUEST_CONTINUE = RequestOperation.REQUEST_CONTINUE;
Base.REQUEST_REDUNDANT = RequestOperation.REQUEST_REDUNDANT;
Base.REQUEST_SKIP = RequestOperation.REQUEST_SKIP;

/**
 * Reserved keywords that can't be used for attribute or option names.
 */
const RESERVED = invert_1([
    '_attributes',
    '_collections',
    '_errors',
    '_listeners',
...
class Model extends Base {
    ...
    memoize() {
        ...
        each(memoized, (name) => this[name] = once_1(this[name]));
    }
    /**
     ...
     */
    getDefaultOptions() {
        return merge_1({}, super.getDefaultOptions(), {
     ...
     */
    compileMutators() {
        this._mutations = mapValues_1(this.mutations(), (m) => flow_1(m));
    }
    /**
     * @returns {Object} Parameters to use for replacement in route patterns.
     */
    getRouteParameters() {
        return merge_1({}, super.getRouteParameters(), this._attributes);
    }
    /**
    ...
    reset(attribute) {
        ...
        if (attribute) {
            copyFrom(this._reference, this._attributes, castArray_1(attribute));
    ...
    mutated(attribute, value) {
        let mutator = get_1(this._mutations, attribute);
        if (mutator) {
    ...
    mutate(attribute) {
        if (isUndefined_1(attribute)) {
            ...
            each(castArray_1(attribute), (attribute) => {
    ...
    sync(attribute) {
        ...
        let active = cloneDeep_1(this._attributes);
        ...
        if (isUndefined_1(attribute)) {
            ...
            each(castArray_1(attribute), (attribute) => {
                Vue.set(this._reference, attribute, get_1(active, attribute));
            });
        }
        this.emit('sync');
    }
    ...
    set(attribute, value) {
        ...
        let changed = defined && !isEqual_1(previous, value);
        if (changed) {
    ...
    unset(attribute) {
        // We're cloning deep to avoid multiple references to the same object,
        // otherwise updating the attributes will also update the reference.
        let defaults = cloneDeep_1(this.defaults());
        // Unset either specific attributes or all attributes if none provided.
        let attributes = defaultTo_1(attribute, keys_1(this._attributes));
        // Unset either specific attributes or all attributes if none provided.
        each(castArray_1(attributes), (attribute) => {
            if (this.has(attribute)) {
                Vue.set(this._attributes, attribute, get_1(defaults, attribute));
            }
        });
    }
    /**
    ...
    validateAttribute(attribute) {
        ...
        return Promise.all(tasks)
            ...
            errors = filter_1(errors, (e) => isString_1(e) || isObject_1(e));
            ...
            if (this.getOption('useFirstErrorOnly') && !isEmpty_1(errors)) {
                return first(errors);
            }
            return errors;
        });
    ...
    validate(attributes) {
        ...
        if (isString_1(attributes)) {
            return this.validateAttribute(attributes).then((errors) => {
                return (!isEmpty_1(errors) ? { [attributes]: errors } : {});
            });
        }
     ...
     */
    addToAllCollections() {
        each(this._collections, (collection) => {
            collection.add(this);
        });
    }
    /**
    ...
    changed() {
        let changed = [];
        each(this._attributes, (value, attribute) => {
            if (!isEqual_1(value, this.saved(attribute))) {
    ...
    update(data) {
        ...
        if (!data || (isObjectLike_1(data) && isEmpty_1(data))) {
            ...
            this.assign(defaults_1({}, data, this._attributes));
    ...
    onSaveSuccess(response) {
        ...
        if (response) {
            ...
            if (response.getStatus() === 201 ||
                (!this.saved('id') && (isPlainObject_1(responseData) && get_1(responseData, 'id')))) {
                action = 'create';
            }
...
class Collection extends Base {
    ...
    constructor(models = [], options = {}, attributes = {}) {
        ...
        this.set(defaultsDeep_1({}, attributes, this.defaults()));
     ...
     */
    reset(...attribute) {
        each(this.models, method_1('reset', ...attribute));
    }
    /**
    ...
    _removeModelAtIndex(index) {
        ...
        let model = get_1(this.models, index);
     ...
     */
    isModel(candidate) {
        return isObject_1(candidate)
            && has_1(candidate, '_attributes')
            && has_1(candidate, '_uid');
    }
    /**
    ...
    applyValidationErrorObject(errors) {
        let lookup = keyBy_1(this.models, (model) => model.identifier());
        each(errors, (errors, identifier) => {
            let model = get_1(lookup, identifier);
            if (model) {
    ...
    page(page) {
        ...
        if (isNil_1(page)) {
            ...
            Vue.set(this, '_page', max_1([1, toSafeInteger_1(page)]));
        }
        return this;
    }
    /**
    ...
    getDeleteQuery() {
        ...
        return { [identifier]: join_1(identifiers, ',') };
    }
    /**
     * Called before a delete request is made.
     *
    ...
    onDelete() {
        ...
        return Promise.all(this.models.map((m) => m.onDelete()))
            .then(() => {
            // No need to do anything if no models should be deleted.
            if (isEmpty_1(this.getDeletingModels())) {
...
function customDefaultsAssignIn(objValue, srcValue, key, object) {
  if (objValue === undefined ||
      (eq_1(objValue, objectProto$o[key]) && !hasOwnProperty$k.call(object, key))) {
    return srcValue;
  }
  return objValue;
}
...
function template(string, options, guard) {
  ...
  var sourceURL = hasOwnProperty$l.call(options, 'sourceURL')
    ? ('//# sourceURL=' +
  ...
  var variable = hasOwnProperty$l.call(options, 'variable') && options.variable;
  if (!variable) {
    source = 'with (obj) {\n' + source + '\n}\n';
  }
...
var _merge = _interopRequireDefault(merge_1$1);
...
const parseDate$1 = (value, format) => {
    if (isString_1(value)) {
...
class GlobalMessages {
     ...
     */
    locale(locale) {
        this.$locale = toLower_1(locale);
    }
    /**
     * Registers a language pack.
     */
    register(bundle) {
        let locale = toLower_1(bundle.locale);
        each(get_1(bundle, 'messages', {}), (message, name) => {
            set_1(this.$locales, [locale, name], template_1(message));
        });
    }
    /**
    ...
    set(name, format, locale) {
        let $template = isString_1(format) ? template_1(format) : format;
        // Use the given locale.
        if (locale) {
            set_1(this.$locales, [locale, name], $template);
            // Otherwise use the active locale.
        }
        else if (this.$locale) {
            set_1(this.$locales, [this.$locale, name], $template);
    ...
    get(name, data = {}) {
        ...
        let template = get_1(this.$locales, [this.$locale, name], get_1(this.$locales, [split_1(this.$locale, '-')[0], name], get_1(this.$locales, [this.$fallback, name])));
...
const rule = function (config) {
    let name = get_1(config, 'name');
    let data = get_1(config, 'data', {});
    let test = get_1(config, 'test', stubTrue_1);
    /**
    ...
    let $rule = function (value, attribute, model) {
        ...
        if (valid) {
            ...
            for (let _or of $rule._or) {
                ...
                if (result === true || isUndefined_1(result)) {
                    return true;
                }
            }
        }
        ...
        assign_1(data, { attribute, value });
        // This would be a custom format explicitly set on this rule.
        let format = get_1($rule, '_format');
     ...
     */
    $rule.copy = () => {
        return assign_1(rule({ name, test, data }), pick_1($rule, [
            '_format',
            '_and',
            '_or',
        ]));
     ...
     */
    $rule.format = (format) => {
        return assign_1($rule.copy(), { _format: format });
    };
    /**
     ...
     */
    $rule.or = (rules) => {
        return assign_1($rule.copy(), { _or: concat_1($rule._or, rules) });
    };
    /**
     ...
     */
    $rule.and = (rules) => {
        return assign_1($rule.copy(), { _and: concat_1($rule._and, rules) });
    };
    $rule._and = []; // "and" chain
    $rule._or = []; // "or" chain
 ...
 */
const after = function (date) {
    return rule({
        name: 'after',
        data: { date },
        test: (value) => isAfter(parseDate$1(value), parseDate$1(date)),
    });
};
/**
...
const ascii = rule({
    name: 'ascii',
    test: (value) => isString_1(value) && /^[\x00-\x7F]+$/.test(value),
});
/**
...
const base64 = rule({
    name: 'base64',
    test: (value) => isString_1(value) && isBase64(value),
});
/**
 * Checks if a value is before a given date string or `Date` object.
 */
const before$1 = function (date) {
    return rule({
        name: 'before',
        data: { date },
        test: (value) => isBefore(parseDate$1(value), parseDate$1(date)),
    });
};
/**
 * Checks if a value is between a given minimum or maximum, inclusive by default.
 */
const between = function (min, max, inclusive = true) {
    let _min = +(isString_1(min) ? parseDate$1(min) : min);
    let _max = +(isString_1(max) ? parseDate$1(max) : max);
    return rule({
        data: { min, max },
        name: inclusive ? 'between_inclusive' : 'between',
        test: (value) => {
            let _value = +(isString_1(value) ? parseDate$1(value) : value);
            return inclusive
                ? gte_1(_value, _min) && lte_1(_value, _max)
                : gt_1(_value, _min) && lt_1(_value, _max);
        },
    });
};
/**
...
const creditcard = rule({
    name: 'creditcard',
    test: (value) => isString_1(value) && isCreditCard(value),
});
/**
 ...
 */
const dateformat = function (format$1) {
    return rule({
        name: 'dateformat',
        data: { format: format$1 },
        test: (value) => {
            try {
                return isValid(parseDate$1(value.toString(), format$1))
                    && format(parseDate$1(value.toString(), format$1), format$1) === value.toString();
            }
            catch (error) {
...
const defined = rule({
    name: 'defined',
    test: (value) => !isUndefined_1(value),
});
/**
...
const email = rule({
    name: 'email',
    test: (value) => isString_1(value) && isEmail(value),
});
/**
...
const equals = function (other) {
    return rule({
        name: 'equals',
        data: { other },
        test: (value) => isEqual_1(value, other),
    });
};
/**
 * Alias for `equals`
 ...
 */
const gt$1 = function (min) {
    return rule({
        name: 'gt',
        data: { min },
        test: (value) => gt_1(value, min),
    });
};
/**
 * Checks if a value is greater than or equal to a given minimum.
 */
const gte$1 = function (min) {
    return rule({
        name: 'gte',
        data: { min },
        test: (value) => gte_1(value, min),
    });
};
/**
...
const ip = rule({
    name: 'ip',
    test: (value) => isString_1(value) && isIP(value),
});
/**
...
const iso8601 = rule({
    name: 'iso8601',
    test: (value) => isString_1(value) && isISO8601(value),
});
/**
...
const json = rule({
    name: 'json',
    test: (value) => isString_1(value) && isJSON(value),
});
/**
 ...
 */
const lt$1 = function (max) {
    return rule({
        name: 'lt',
        data: { max },
        test: (value) => lt_1(value, max),
    });
};
/**
 * Checks if a value is less than or equal to a given maximum.
 */
const lte$1 = function (max) {
    return rule({
        name: 'lte',
        data: { max },
        test: (value) => lte_1(value, max),
    });
};
/**
 * Checks if a value matches a given regular expression string or RegExp.
 */
const match$1 = function (pattern) {
    return rule({
        name: 'match',
        data: { pattern },
...
const negative = rule({
    name: 'negative',
    test: (value) => toNumber_1(value) < 0,
});
/**
 *
 */
const not = function (...values) {
    return rule({
        name: 'not',
        test: (value) => !includes_1(values, value),
    });
};
/**
...
const numeric = rule({
    ...
    test: (value) => {
        return (isNumber_1(value) && !_isNaN(value))
            || (value && isString_1(value) && !_isNaN(toNumber_1(value)));
    },
});
/**
...
const required = rule({
    name: 'required',
    test: (value) => !(isNil_1(value) || value === ''),
});
/**
...
const same = function (other) {
    return rule({
        name: 'same',
        data: { other },
        test: (value, attribute, model) => isEqual_1(value, model.get(other)),
    });
};
/**
...
const url = rule({
    name: 'url',
    test: (value) => isString_1(value) && isURL(value),
});
/**
...
const uuid = rule({
    name: 'uuid',
    test: (value) => isString_1(value) && isUUID(value),
});

export { Collection, Model, af_za, after, alpha, alphanumeric, ar_iq, array, ascii, base64, before$1 as before, between, boolean, creditcard, da_dk, date, dateformat, de_de, defined, email, empty, en_us, equal, equals, es_es, fa_ir, fr_fr, gt$1 as gt, gte$1 as gte, id_id, integer, ip, isblank, isnil, isnull, iso8601, json, length, lt$1 as lt, lte$1 as lte, match$1 as match, max$1 as max, messages, min, negative, nl_nl, not, number, numeric, object, pl_pl, positive, pt_br, required, ru_ru, rule, same, string, url, uuid };
